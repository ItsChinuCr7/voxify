    ...
    if (paginatedUsers.length === 0) {
        ...
        tbody.innerHTML = paginatedUsers.map(user => `
            <tr>
                ...
                <td>
                    <button class="action-btn edit-btn" onclick="openEditUserModal(${user.id})">✏️ Edit</button>
                    <button class="action-btn delete-btn" onclick="openDeleteModal(${user.id})">🗑️ Delete</button>
                </td>
            </tr>
        `).join('');
    }
